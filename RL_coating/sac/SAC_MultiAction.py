...
from .networks import HybridPolicyNetwork, QNetwork, QNetworkNoAction
from .SAC import SAC
...
class SAC_Discrete(SAC):
    ...
    def __init__(
        ...
        self.actor = HybridPolicyNetwork(state_dim, discrete_action_dim, hidden_size)
        ...
        self.critic1_optimizer = torch.optim.Adam(list(self.critic1.parameters()), lr=3*self.lr)
        self.critic2_optimizer = torch.optim.Adam(list(self.critic2.parameters()), lr=3*self.lr)
    ...
    def produce_action_and_action_info(self, state):
        ...
        """
        discrete_action_probabilities, continuous_mean, continuous_log_std = self.actor(state)
        max_probability_action = torch.argmax(discrete_action_probabilities, dim=-1)
        action_distribution = torch.distributions.Categorical(discrete_action_probabilities)
        action = action_distribution.sample().cpu()
        # Have to deal with situation of 0.0 probabilities because we can't do log 0
        z = discrete_action_probabilities == 0.0
        z = z.float() * 1e-8
        log_action_probabilities = torch.log(discrete_action_probabilities + z)

        std = continuous_log_std.exp()
        normal = TruncatedNormalDist(continuous_mean, std, self.gaussian_lower_bound, self.gaussian_upper_bound)
        ...
        action_log_probabilities = log_prob + log_action_probabilities
