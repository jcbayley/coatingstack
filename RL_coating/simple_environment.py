...
class CoatingStack():
    ...
    def sample_action_space(self, ):
        ...
        if self.discrete and not self.continuous:
            ...
            new_layer_thickness = 0.1
        if self.continuous:
            new_layer_thickness = torch.random.uniform(self.min_thickness, self.max_thickness)
            new_layer_material = torch.from_numpy(np.array([1,0,0]))
        if self.continuous and self.discrete:
            new_layer_thickness = torch.random.uniform(self.min_thickness, self.max_thickness)
            new_layer_material = torch.nn.functional.one_hot(torch.from_numpy(np.array(np.random.randint(self.n_materials))), num_classes=self.n_materials)
    ...
    def compute_state_value(
        ...
        for i, layer in enumerate(state):
            ...
            if material_ind == 1:
                ...
                val = 0.65
            #m_scaled += -np.clip(np.abs((val - state[i][0])**2), -1, 1)
                
            m_scaled += 1/(1+np.abs((val - state[i][0])**2))

            if i == 0:
                m_scaled += (material["n"]/self.materials[1]["n"])/self.max_material_ratio
            else:
    ...
    def update_state(self, current_state, action):
        ...
        material = torch.nn.functional.one_hot(torch.from_numpy(np.array([action[0]]).astype(int)), num_classes=self.n_materials)[0]
        thickness = torch.from_numpy(np.array([action[1]]))#[0]
    ...
    def step(self, action, max_state=0, verbose=False):
        ...
        if self.continuous and not self.discrete:
            action = [1, action]
        elif self.discrete and not self.continuous:
            action = [action, [0.1]]

        #print(action)
        thickness_change = action[1]#[0]
        material_change = action[0]

   
        new_state, full_action = self.update_state(np.copy(self.current_state), action)
      
        reward = 0
        new_value = 0

        reward_diff, reward, new_value = self.compute_reward(new_state, max_value=max_state)
        ...
        if thickness_change <=0:
            terminated=True
            reward = -5
        elif self.current_index >= self.max_layers-1 or material_change == 0:
        ...
        #if material_change == self.previous_material:
        #    reward -= 0.1

        self.previous_material = material_change
        #print(new_value)
